...
use dept_starter_kit_template::docs::{add_doc_schema, close_doc, create_doc, delete_entry, drop_doc, fetch_doc_as_json, get_document, get_download_policy, get_entries, get_entry, get_entry_blob, join_doc, leave, list_docs, save_as_doc, set_download_policy, set_entry, set_entry_file, share_doc, status, ImportFileOutcome};
use dept_starter_kit_template::utils::{decode_doc_id, SS58AuthorId};
use dept_starter_kit_template::blobs::{add_blob_bytes, add_blob_named, add_blob_from_path, list_blobs, has_blob, get_blob, status_blob, list_tags, delete_tag, export_blob_to_file, download_blob, download_hash_sequence, download_with_options};
use dept_starter_kit_template::authors::{list_authors, get_default_author, set_default_author, create_author, delete_author, verify_author};
...
//     let add_blob_bytes_outcome_2 = add_blob_bytes(
//         state.blobs.clone(), 
//         "Hello, world! My name is Mudit Sarda".as_bytes()
//     ).await?;
//     println!("Bytes added to blob: {:?}\n", add_blob_bytes_outcome_2);

//     let add_blob_bytes_outcome_3 = add_blob_bytes(
//         state.blobs.clone(), 
//         "Hello, world! My name is Mudit Sarda. I work at Dhiway.".as_bytes()
//     ).await?;
//     println!("Bytes added to blob: {:?}\n", add_blob_bytes_outcome_3);
